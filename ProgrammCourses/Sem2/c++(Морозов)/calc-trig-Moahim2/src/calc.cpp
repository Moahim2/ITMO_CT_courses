...
void convert_to_RAD(double & x, bool rad_on)
{
    if (!rad_on) {
        x = (x * M_PI) / 180;
    }
...
void convert_to_DEG(double & x, bool rad_on)
{
    if (!rad_on) {
        x = (x * 180) / M_PI;
    }
...
double unary(double current, const Op op, bool & rad_on)
{
    ...
    case Op::CTN:
        convert_to_RAD(current, rad_on);
        if (tan(current) == 0.0) {
            return +INFINITY; //Не придумал как лучше возвращать inf а тесты этого требуют,
        }                     //причем просто 1/tan или cos/sin запрещает UnBehavior, а tan(pi/2 - x) потеря точности
        return 1 / tan(current);
    case Op::ASIN:
        if (current >= -1 && current <= 1) {
            current = asin(current);
            convert_to_DEG(current, rad_on);
        }
        else {
