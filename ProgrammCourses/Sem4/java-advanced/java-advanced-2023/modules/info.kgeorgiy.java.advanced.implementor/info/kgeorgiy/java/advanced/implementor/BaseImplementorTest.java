...
public class BaseImplementorTest extends BaseTest {
    ...
    protected static void check(final URLClassLoader loader, final Class<?> token) {
        ...
        if (token.isInterface()) {
            Assert.assertTrue(name + " should implement " + token, token.isAssignableFrom(impl));
        } else {
            Assert.assertSame(name + " should extend " + token, token, impl.getSuperclass());
        }
    ...
    protected void test(final boolean shouldFail, final Class<?>... classes) {
        test(Paths.get(testMethodName), shouldFail, classes);
    }
}
