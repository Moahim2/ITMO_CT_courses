...
public class MyScanner {
    ...
    public MyScanner(InputStream sourse) throws UnsupportedEncodingException {
    ...
    public int nextDigInt() throws IOException {
        if (this.finish) {
            throw new IllegalStateException("This MyScanner is closed");
        }
        if (!goToNextInt()) {
        ...
        while (isDigit(buffer[this.bufferBegin])) {
            sb.append(buffer[this.bufferBegin]);
            if (this.bufferBegin < this.read - 1) {
                this.bufferBegin++;
            } else {
    ...
    public boolean goToNextDigInt() throws IOException {
        if (this.finish) {
            throw new IllegalStateException("This MyScanner is closed");
        }
        while (this.read != -1) {
    ...
    public boolean goToNextWordSymbol() throws IOException {
        if (this.finish) {
            throw new IllegalStateException("This MyScanner is closed");
        }
        while (this.read != -1) {
            for (int i = this.bufferBegin; i < this.read; i++) {
                if (isWordSymbol(buffer[i])) {
                    this.bufferBegin = i;
                    return true;
                }
                this.lastChar = buffer[i];
